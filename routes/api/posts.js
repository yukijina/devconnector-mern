...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    // Check user - only user's own post can be deleted, not other posts
...
router.put('/unlike/:postId', auth, async (req, res) => {
  try {
    // params is set :postId so we get req.params.postId (not just req.params.id)
    const post = await Post.findById(req.params.postId);
    // Check if the post has already been liked
    // video shows .length > 0 after req.user )
...
// @route DELETE api/posts/comment/:id(postId)/:commentId
// @desc  Delete a posts
// @access Private
router.delete('/comment/:id/:commentId', auth, async (req, res) => {
  try {
    ...
    if (comment.user.toString() !== req.user.id) {
      return res.status(404).json({ meg: 'Comment does not exist' });
    }
